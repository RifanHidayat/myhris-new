...
class monitoringPulangCepat extends StatefulWidget {
  String? em_id, full_name;
  monitoringPulangCepat({Key? key, this.em_id, this.full_name})
      : super(key: key);
  @override
  _monitoringPulangCepatState createState() => _monitoringPulangCepatState();
}

class _monitoringPulangCepatState extends State<monitoringPulangCepat> {
  var controller = Get.put(MonitoringController());

  @override
  void initState() {
    controller.loadMonitoringPulangCepat();
    ...
    return Obx(
      () => ListView.builder(
          ...
          itemBuilder: (context, index) {
            ...
            print("id ${controller.historyAbsen.value[index]}");
  ...
  Widget tampilan2(MonitoringDataModel index) {
    ...
    var jamPulang = index.jamPulang ?? '00:00:00';
    ...
    DateTime waktuAwal = DateTime.parse("${index.attenDate} ${jamKeluar}");
    DateTime waktuAkhir = DateTime.parse("${index.attenDate} ${jamPulang}");
 ...
 int totalDetik = selisih.inSeconds % 60;
    ...
    if (totalMenit > 0 && totalDetik > 0) {
      ...
      if (totalMenit>0){
        //   text =
        // "Terlambat ${totalMenit} Menit sebelum waktu masuk";
         text =
