...
import 'package:siscom_operasional/controller/laporan_klaim_controller.dart';
...
class LaporanKlaimDetail extends StatefulWidget {
  ...
  LaporanKlaimDetail(
      ...
      : super(key: key);
  @override
  _LaporanKlaimDetailState createState() => _LaporanKlaimDetailState();
}

class _LaporanKlaimDetailState extends State<LaporanKlaimDetail> {
  var controller = Get.put(LaporanKlaimController());
  ...
  Widget listAjuanTidakMasukEmployee() {
    return Obx(
      () => ListView.builder(
          ...
          itemBuilder: (context, index) {
            return viewKlaim(controller.listDetailLaporanEmployee.value[index]);
          }),
    );
  }

  Widget viewKlaim(index) {
    var nomorAjuan = index['nomor_ajuan'];
    var tanggalPengajuan = index['created_on'];
    var status;
